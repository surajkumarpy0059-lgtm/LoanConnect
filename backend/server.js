...
app.post('/api/customer/request-otp',(req,res)=>{const m=String(req.body.mobile||'');if(!/^\d{10}$/.test(m))return res.status(400).json({error:'Invalid mobile'});res.json({ok:true,message:'OTP provider must be connected for production'});});
app.post('/api/customer/verify-otp',(req,res)=>{const m=String(req.body.mobile||''),o=String(req.body.otp||'');if(!/^\d{10}$/.test(m))return res.status(400).json({error:'Invalid mobile'});if(!process.env.DEMO_OTP||o!==process.env.DEMO_OTP)return res.status(401).json({error:'OTP verification unavailable or invalid'});db.prepare('INSERT OR IGNORE INTO customers(mobile,created_at) VALUES(?,?)').run(m,now());res.json({token:token('customer',m),mobile:m});});
